...
/**
 * Implementation of IBaseRepository for JPA Created by Rick on 4/14/17.
 */
public class BaseRepository<T extends Identity> implements IBaseRepository<T>, AutoCloseable {
	...
	public void remove(Collection<T> entities) {
		runInTransaction(entityManager -> {
			entities.stream().map(Identity::getId).map(id -> entityManager.find(type, id)).filter(Objects::nonNull)
	...
	public void runNamedStoredProcedure(String name) {
		run(entityManager -> {
			StoredProcedureQuery spQuery = entityManager.createNamedStoredProcedureQuery(name);
			spQuery.execute();
		});
	}
